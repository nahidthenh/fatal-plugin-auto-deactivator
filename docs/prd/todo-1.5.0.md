...
> **Status: shipped in 1.5.0 — all tasks complete.** Adversarial review during implementation changed some task-text details (alert ordering vs the error page, per-channel cooldown state, watchdog damping fields, webhook hardening); the authoritative record is each PRD's "Implementation notes — as shipped in 1.5.0" section and reference.md.
...
- [x] Empty/missing option returns full defaults from both methods
- [x] Malformed option values (wrong types) normalize to defaults
...
- [x] Saved values round-trip; invalid emails dropped silently; empty recipients ⇒ falls back to `admin_email` at send time (store `''`)
...
- [x] Crash-test fatal with email enabled ⇒ one email naming plugin, status verb, error, file:line
- [x] Same fatal repeated within cooldown ⇒ no second email; `fpad_alert_state` holds one fingerprint
- [x] Status not in `notify_statuses` ⇒ no send, no state write
...
### Checkpoint 1
- [x] `php -l` sweep clean; defaults-off upgrade shows **zero** behavior change vs 1.4.0 (no emails, no new option rows until enabled)
- [x] Email vertical works end-to-end on the dev site
...
- [x] Local endpoint (e.g. `nc -l` / webhook.site) receives exact FR-4 JSON once per cooldown window
- [x] Slack format posts render in a real Slack/Discord-slash-compatible webhook
- [x] Invalid/http URL rejected on save with settings error
...
- [x] Parse error in an early-loading mu-plugin-style crash ⇒ entry lands in queue; next normal request delivers it and empties the queue
- [x] Lock prevents double-send under two concurrent requests (best-effort check)
...
- [x] Never-enabled install: `wp cron event list` shows no fpad events
- [x] Enable → event exists; deactivate → gone; uninstall → both options gone
...
### Checkpoint 2
- [x] Alerts acceptance criteria 1–7 + 10 from the PRD all pass
- [x] Fatal with both channels enabled delays the visitor error page imperceptibly (webhook non-blocking confirmed)
...
- [x] Test email arrives; test webhook fires; failure shows response code in the notice
...
- [x] Crash via `memory_limit` exhaustion (e.g. `str_repeat` bomb in the test plugin at a low `memory_limit`) still produces a log entry and an alert
- [x] `OWNERSHIP_MARKER` string still present in both variants
...
- [x] Returns `array( 'status', 'detail' )`; all four legacy statuses pass through unchanged
- [x] Define the constant in wp-config ⇒ `disabled`; rename the plugin dir ⇒ `stranded`
...
- [x] Both new statuses render in banner + notice + Site Health as critical, with actionable text
- [x] `active` sites see no change
...
- [x] Delete drop-in, `wp cron event run fpad_watchdog_check` ⇒ reinstalled, state `active`, no alert
- [x] Foreign file twice in a row ⇒ one reclaim, then back-off (no reinstall loop), state records it
- [x] Upgrade-without-reactivation self-schedules on next `admin_init`
...
- [x] Lost→alert once, second run within 24 h silent, restore→recovery alert
- [x] Works with alerts feature fully unconfigured (wp_mail fallback)
...
- [x] Both render; absent state (pre-first-run) shows an em dash, not a PHP notice
- **Verify:** fresh install before/after first cron run
- **Deps:** W3 · **Files:** `includes/class-admin.php` · **Size:** XS

### Checkpoint 3
- [x] Watchdog PRD acceptance criteria 1–7 pass
- [x] Combined soak: healthy site runs watchdog + drain crons for a day with zero option churn (state written only on change) and zero notices in `debug.log`
...
- [x] Every new symbol in this TODO appears in reference.md; sync-pairs table covers every mirror introduced
...
- [x] FAQ claims match shipped behavior exactly (no overpromising on delivery guarantees)
...
- [x] Three version spots match; POT contains the new settings-UI strings
- **Deps:** R2 · **Size:** XS
...
- [x] All checkpoints re-verified on the release build zip (build-archive workflow)
