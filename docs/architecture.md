...
- **The earliest-boot window is a supported case.** WP registers the fatal error handler at `wp-settings.php`'s `wp_register_fatal_error_handler()`, but loads `formatting.php` (escaping), `functions.php` → `option.php` (the options API), and `wp_plugin_directory_constants()` (`WP_PLUGIN_DIR`) *after* it. A fatal in `advanced-cache.php`, `object-cache.php`, `db.php`, `sunrise.php`, or an early core file therefore reaches `handle()` with those missing — and on PHP 8 reading an undefined constant **throws**. `match_active_plugin()` bails when `WP_PLUGIN_DIR` is undefined, `get_active_plugins()` bails when `get_option()` is, and the page escapes through `esc()`/`esc_link()`. Net effect: the fatal is reported as `unattributed` (correctly — no plugin has loaded that early) and the error page still renders, instead of a silent blank 200.
