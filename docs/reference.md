...
| `match_active_plugin( $error )` | protected | Normalized prefix match of `$error['file']` against each active plugin's directory (single-file plugins matched exactly, symlinks resolved); returns basename or `''`. No side effects. Returns `''` immediately when `WP_PLUGIN_DIR` is undefined (fatal before `wp_plugin_directory_constants()`; reading it would throw on PHP 8) |
