...
`FPAD_Fatal_Error_Handler::match_active_plugin()` is the whole algorithm (normalized path prefix match; single-file plugins matched exactly). If you change *source semantics* (what counts as plugin/theme/core), update both `detect_error_source()` and its admin mirror `FPAD_Admin::source_key()`.
